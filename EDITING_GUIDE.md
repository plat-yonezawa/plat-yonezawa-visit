...
### 連絡先と予約リクエストの扱い（この2本だけ他と違う）

黒獅子2本は**やまがたアルカディア観光局の取扱商品**で、PLAT YONEZAWA は資料を掲載している側。
そのため他の体験ページと次の2点が異なる。**揃えようとして戻さないこと。**

- **予約リクエストフォーム（HubSpot）を載せていない。** 予約は観光局が直接受けるため、
  CTA はすべて `#contact` へのアンカーで、そこから `info@arcadia-kanko.jp` へのメールに繋がる。
- **連絡先は `info@arcadia-kanko.jp` / 0238-88-1831。** ガイド 7.（`info1@plat-yonezawa.jp` で統一）
  の例外。取扱事業者が別なので、問い合わせが観光局に直接届く形にしている。
- 「アルカディア観光局の商品」と分かるように、ヒーローの提供事業者欄・仕様表の先頭行・
  フッター（催行販売＝観光局／資料掲載＝プラットヨネザワ）の3か所で明示している。
